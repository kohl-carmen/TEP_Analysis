...
%         t=t+1
%         EEG.times(t)
%         abs(EEG.data(electr_oi_i,t+1,trial))
%         abs(EEG.data(electr_oi_i,t+1,trial))> abs(EEG.data(electr_oi_i,t,trial).*scale)%EEG.data(electr_oi_i,t+1,trial)> EEG.data(electr_oi_i,t+1,trial).*10               
        if EEG.times(t)>tms_time_wndw(1) & EEG.times(t)<tms_time_wndw(2) & just_found_one==0
            if  abs(EEG.data(electr_oi_i,t+1,trial))> basemean+basesd*scale%EEG.data(electr_oi_i,t+1,trial)> EEG.data(electr_oi_i,t+1,trial).*10
                pulse_onset(trial,min(find(found_pulse==0)))=EEG.times(t);
                found_pulse(min(find(found_pulse==0)))=1;
                just_found_one=just_found_one+1;
            end
        end
    end
    if sum(found_pulse)<3
        fprintf(' Couldnt find 3 pulses in trial %d\n',trial)
    end
end
%sanity check - find times that are different from the rest
pulsemean=mean(pulse_onset);
pulsesd=[2 2 2];%std(pulse_onset).*2; % I just put no further away than 2ms rather than an sd caus sds are tiny
redo_trials=[];
redo_trials=[redo_trials; find(pulse_onset(:,1)>pulsemean(1)+pulsesd(1) | pulse_onset(:,1)<pulsemean(1)-pulsesd(1) )];
redo_trials=[redo_trials; find(pulse_onset(:,2)>pulsemean(2)+pulsesd(2) | pulse_onset(:,2)<pulsemean(2)-pulsesd(2) )];
redo_trials=[redo_trials; find(pulse_onset(:,3)>pulsemean(3)+pulsesd(3) | pulse_onset(:,3)<pulsemean(3)-pulsesd(3) )];
if ~isempty(redo_trials)
    fprintf('%d trials were found to have odd pulse times \n',size(redo_trials,1))
    pulse_onset(redo_trials,:)
else
    fprintf('Pulses where found for all trials.\nMean pulse times are:\t%d\t%d\t%d\n',round(pulsemean))
end
...
count=0;
for trial=1:size(EEG.data,3)
    pulse1=[find(EEG.times==pulse_onset(trial,1))+round(pulse_zero_time(1)*dt): find(EEG.times==pulse_onset(trial,1))+round(pulse_zero_time(2)*dt)];
    pulse2=[find(EEG.times==pulse_onset(trial,2))+round(pulse_zero_time(1)*dt): find(EEG.times==pulse_onset(trial,2))+round(pulse_zero_time(2)*dt)];
    pulse3=[find(EEG.times==pulse_onset(trial,3))+round(pulse_zero_time(1)*dt): find(EEG.times==pulse_onset(trial,3))+round(pulse_zero_time(2)*dt)];
    ...
    if plot_steps==true
        ...
        if any(trial==trials_to_plot)
            ...
            hold on
            plot(pulse_onset(trial,:),EEG.data(electr_oi_i,[find(EEG.times==pulse_onset(trial,1)),find(EEG.times==pulse_onset(trial,2)),find(EEG.times==pulse_onset(trial,3))],trial),'ro')
...
%         EEG.data(electr_oi_i,t+1,trial)
        if EEG.times(t)>tms_time_wndw(1) & EEG.times(t)<tms_time_wndw(2) & just_found_one==0
            if  abs(temp(t+1))> basesd*scale %& abs(EEG.data(electr_oi_i,t+1,trial))-abs(EEG.data(electr_oi_i,t,trial))>20
                recharge_onset(trial,min(find(found_recharge==0)))=EEG.times(t);
                found_recharge(min(find(found_recharge==0)))=1;
                just_found_one=just_found_one+1;
            end
        end
    end
    if sum(found_recharge)<3
        missing=[missing,trial];
        fprintf(' Couldnt find 3 recharges in trial %d\n',trial)
    end
end

% check if those times make sense
rechargemean=mean(recharge_onset);
rechargesd=[2 2 2];%std(pulse_onset).*2; % I just put no further away than 2ms rather than an sd caus sds are tiny
redo_trials=[];
redo_trials=[redo_trials; find(recharge_onset(:,1)>rechargemean(1)+rechargesd(1) | recharge_onset(:,1)<rechargemean(1)-rechargesd(1) )];
redo_trials=[redo_trials; find(recharge_onset(:,2)>rechargemean(2)+rechargesd(2) | recharge_onset(:,2)<rechargemean(2)-rechargesd(2) )];
redo_trials=[redo_trials; find(recharge_onset(:,3)>rechargemean(3)+rechargesd(3) | recharge_onset(:,3)<rechargemean(3)-rechargesd(3) )];
...
if ~isempty(redo_trials)
    ...
    for i=1:length(redo_trials)
        ...
        hold on
        plot(EEG.times([1000*dt:1080*dt]),EEG.data(electr_oi_i,[1000*dt:1080*dt],redo_trials(i)))
        plot(recharge_onset(redo_trials(i),:),[0 0 0], 'ro')
    end
    ...
    while ~isempty(redo_trials) & scale<20
                ...
                if EEG.times(t)>tms_time_wndw(1) & EEG.times(t)<tms_time_wndw(2) & just_found_one==0
                    if  abs(EEG.data(electr_oi_i,t+1,redo_trials(trial)))> basemean+basesd*scale
                        if any(EEG.data(electr_oi_i,t+1:t+4,redo_trials(trial))~=0) & any(EEG.data(electr_oi_i,t-2:t,redo_trials(trial))~=0)
                            recharge_onset(redo_trials(trial),min(find(found_recharge==0)))=EEG.times(t);
                            found_recharge(min(find(found_recharge==0)))=1;
                            just_found_one=just_found_one+1;
                        end
                    end
        ...
        for i=1:length(old_redo_trials)
            ...
            hold on
            plot(EEG.times([1000*dt:1080*dt]),EEG.data(electr_oi_i,[1000*dt:1080*dt],old_redo_trials(i)))
            plot(recharge_onset(old_redo_trials(i),:),[0 0 0], 'ro')
        end
        pause(1)
...
count=0;
for trial=1:size(EEG.data,3)
    recharge1=[find(EEG.times==recharge_onset(trial,1))+round(recharge_zero_time(1)*dt): find(EEG.times==recharge_onset(trial,1))+round(recharge_zero_time(2)*dt)];
    recharge2=[find(EEG.times==recharge_onset(trial,2))+round(recharge_zero_time(1)*dt): find(EEG.times==recharge_onset(trial,2))+round(recharge_zero_time(2)*dt)];
    recharge3=[find(EEG.times==recharge_onset(trial,3))+round(recharge_zero_time(1)*dt): find(EEG.times==recharge_onset(trial,3))+round(recharge_zero_time(2)*dt)];
    if interp==true
        for electr=1:size(EEG.data,1)
        %     clf
        %     plot(EEG.times(999*dt:1020*dt), EEG.data(electr,[999*dt:1020*dt],1))
                ...
                y=EEG.data(electr,:,trial);
                x=EEG.times;
                x([recharge1,recharge2,recharge3])=[];
                y([recharge1,recharge2,recharge3])=[];
                xx=EEG.times([recharge1,recharge2,recharge3]);   
                yy=interp1(x,y,xx,'pchip');%pchip is cubib
                EEG.data(electr,[recharge1,recharge2,recharge3],trial)=yy;
        %     plot(xx, yy,'.')
         end
    elseif zero==true
        EEG.data(:,[recharge1,recharge2,recharge3],trial)=0;
        %plot(EEG.times(pulse2),zeros(size(pulse1)))
    elseif medianfilt==true
        for electr=1:size(EEG.data,1)
            filt_ord =length(recharge1);%number of datapoints considered left and right
            EEG.data(electr,[recharge1],trial)=medfilt1(EEG.data(electr,[recharge1],trial),filt_ord,'truncate');
            EEG.data(electr,[recharge2],trial)=medfilt1(EEG.data(electr,[recharge2],trial),filt_ord,'truncate');
            EEG.data(electr,[recharge3],trial)=medfilt1(EEG.data(electr,[recharge3],trial),filt_ord,'truncate');
        end
    end
    ...
    if plot_steps==true
        ...
        if any(trial==trials_to_plot)
            ...
            subplot(nr_trials_to_plot,1,count)
            hold on
            plot(recharge_onset(trial,:),EEG.data(electr_oi_i,[find(EEG.times==recharge_onset(trial,1)),find(EEG.times==recharge_onset(trial,2)),find(EEG.times==recharge_onset(trial,3))],trial),'bo')        
            plot(EEG.times(find(EEG.times==plot_times(1)):find(EEG.times==plot_times(2))),EEG.data(electr_oi_i,[find(EEG.times==plot_times(1)):find(EEG.times==plot_times(2))],trial))
            ylim([min(EEG.data(electr_oi_i,[find(EEG.times==plot_times(1)):find(EEG.times==plot_times(2))],trial)),max(EEG.data(electr_oi_i,[find(EEG.times==plot_times(1)):find(EEG.times==plot_times(2))],trial))])
        end
    end
...
%% remove bad channels
% figure; pop_spectopo(nudel, 1, [], 'EEG' , 'percent',15,'freq', [10 20 30],'freqrange',[2 80],'electrodes','off');

if partic==2
    bad=[12 20 47 54 55 57];
elseif partic==4
    bad=[1 14 20 29 46 52 57];
end
...
%pop_eegplot( EEG, 1, 1, 1);
if partic==2
    EEG = pop_rejepoch( EEG, [2 4 7 9 10 12:2:16 17 20 22 23 26 31 42 46 49 51 57 65 77 78 81 83 88 94 96 99 100] ,0);
elseif partic==4
    EEG = pop_rejepoch( EEG, [3 19 26 41 43 47 49 55 68 76 80 88 98 101 104 105 107 110 113 115 117 120 121 134 139 147 152 156 162] ,0);
end
